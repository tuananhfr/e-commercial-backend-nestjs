...
export class OrdersService {
  ...
  async checkout(body: checkoutDtoArr, user: Record<string, any>) {
    try {
      ...
      const session = await this.stripeClient.checkout.sessions.create({
        ...
        success_url: `${configuration.frontendUrl}/success`,
        cancel_url: `${configuration.frontendUrl}/cancel`,
      });
      ...
      if (event.type === 'checkout.session.completed') {
        ...
        if (session.payment_status === paymentStatus.paid) {
          ...
          this.sendOrderEmail(
            ...
            orderData.orderId,
            `${configuration.emailService.emailTemplates.orderSuccess}${order._id}`,
          );
        }
      } else {
  ...
  async sendOrderEmail(email: string, orderId: string, orderLink: string) {
    await sendEmail(
      email,
      configuration.emailService.emailTemplates.orderSuccess,
